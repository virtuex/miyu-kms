...
USE miyu_kms;
--
-- 后台管理部分复用“若依”后台管理框架-微服务版本，下面注释掉，以后也可以自己定制页面
-- 用户表
...
--     USER_TENANT_ID BIGINT(20)  NOT NULL COMMENT '租户ID',
...
(
    ...
    HSM_IDENTIFIER VARCHAR(255) NOT NULL UNIQUE COMMENT '密码机标识',
    HSM_HOST     VARCHAR(255)  COMMENT '密码机地址',
    HSM_PORT     BIGINT(20)   COMMENT '密码机端口',
    ...
    HSM_STATUS   VARCHAR(32)         NOT NULL COMMENT '密码机状态',
    CREATED_BY   BIGINT(20)          NOT NULL COMMENT '创建人',
    CREATED_TIME DATETIME     NOT NULL COMMENT '创建时间',
    UPDATED_BY   BIGINT(20)   COMMENT '更新人',
    UPDATED_TIME DATETIME COMMENT '更新时间',
...
(
    USER_HSM_ID BIGINT(20)         NOT NULL COMMENT '授权表主键值',
    USER_HSM_HSM_ID BIGINT(20)         NOT NULL COMMENT '密码机id',
...
-- 添加默认管理员数据
-- INSERT INTO `T_USER` VALUES (1561629900118687746,'admin','ADMIN','$2a$10$zK3OSE4nctibSFupK7MiI.oFU.JIgB48BEwcTIS5kPDUk3HwzkipW',-1,'2022-08-22 16:22:36',-1,-1,'2022-08-22 16:22:36','管理员');